...
DB_PORT = 5432
DB_HOST = 'localhost'
DB_NAME = 'PROYECTO SQL'

# 1) Connect to the database here using the SQLAlchemy's create_engine function
conexion = pymysql.connect(host= DB_HOST, user=DB_USER, password=DB_PASSWORD, port=DB_PORT)
cursor = conexion.cursor(f"CREATE DATA BASE {DB_NAME}")
...
cursor.execute(Crear_tablas_nuevas)
conexion.commit()
...
cursor.execute(insertar_tablas)
conexion.commit()
# 4) Use pandas to print one of the tables as dataframes using read_sql function
sql_query = "SELECT * FROM Crear_tablas_nuevas"
